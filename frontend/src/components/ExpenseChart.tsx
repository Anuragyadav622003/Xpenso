        ...
        <div className="bg-white border rounded-lg p-3 shadow-lg text-gray-800">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.dataKey === 'credit' ? 'Income' : 'Expenses'}: ₹{entry.value?.toFixed(2) || 0}
            </p>
          ))}
        </div>
      );
    ...
    if (active && payload?.length) {
      return (
        <div className="bg-white border rounded-lg p-3 shadow-lg text-gray-800">
          ...
          <p className="text-sm">₹{payload[0].value?.toFixed(2) || 0}</p>
        </div>
      );
    }
    return null;
          ...
          <Card className="shadow-md rounded-2xl">
            ...
            <CardContent>
              {categoryData.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-[400px] text-gray-500 text-center">
                  <div>
